...
def download_and_unzip(name, weburl):
    ...
    try:
        ...
        golm_file, headers = urllib.request.urlretrieve(weburl, "{}.zip".format(name))
    except Exception as e:
        print("Error downloading Golm: {}".format(e), file=sys.stderr)
        exit(1)

    try:
        print("Unpacking {} ...".format(name))
        zip = zipfile.ZipFile(golm_file, 'r')
        dirname = zip.namelist()[0]  # golem-master
...
def golm_init(bot_name=None, do_clone=False):
    ...
    print("Creating a Golm project in {} ...".format(bot_name))
    ...
    if do_clone:
        ...
        download_and_unzip('Golm', "https://github.com/prihoda/golem/archive/master.zip")
    ...
    print("You can start the server inside the directory with: golm start [django-app]")


def golm_start(djangoapp):
    # TODO handle errors
...
def main():
    ...
    argp.add_argument("--clone", action="store_true",
                      help="Downloads Golm from master instead of using PyPI. Only for development, don't use!")
